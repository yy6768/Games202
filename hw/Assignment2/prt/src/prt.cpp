            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    float dw = CalcArea(x + 0.5f, y + 0.5f, width, height);
                    for (int l = 0; l <= SHOrder; l++) {
                        for (int m = -l; m <= l; m++) {
                            double sh_i = sh::EvalSH(l, m, Vector3d(dir.x(), dir.y(), dir.z()).normalized());
                            SHCoeffiecents[sh::GetIndex(l, m)] += sh_i * dw * Le;
                        }
                    }
                }
            }
